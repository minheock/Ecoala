					...
					<div class="board-list"
						...
						<div class="board-main" style="margin-top: 1%; margin-bottom: 1%;">
							<div class="tbl-header">
								<table cellpadding="0" cellspacing="0" border="0">
									...
									<thead >
										<tr>
											<th class="border-radius-left">No</th>
											<th>작성자</th>
											<th class="table_title_line">제목</th>
											<th>작성일</th>
											<th>조회수</th>
											<th class="border-radius-right">좋아요</th>
										</tr>
									</thead>
								</table>
							</div>
								...
								<table cellpadding="0" cellspacing="0" border="0">
									...
									<tbody>
										<c:forEach items="${noticeBoList }" var="board">
											<tr class="text-center" style="background-color: #cfd2f7; ho">
												<td>${board.boardNo }</td>
												<td>관리자</td>
												<td class="text-left boardTitle"><a class="boardTitleTag" style="text-decoration: none; color: black"
													href="<c:url value="/admin/mngrBoardDetView?boardNo=${board.boardNo }"/> ">[공지] ${board.boardTitle }</a>
												</td>
										...
										<c:forEach items="${boardList }" var="board">
											<tr class="text-center">
												...
												<td>${board.userId }</td>
												<td class="text-left boardTitle"><a class="boardTitleTag" style="text-decoration: none; color: black"
													href="<c:url value="/admin/mngrBoardDetView?boardNo=${board.boardNo }"/> ">${board.boardTitle }</a>
												</td>
												<td>${board.boardModDt eq null ? board.boardModDt:board.boardModDt }</td>
												<td>${board.boardHit }</td>
												<td>${board.likeCount }</td>
						...
						<div class="container board-bottom"
							...
							<div style="justify-content: flex-end; margin-left: auto;">
								<a href="<c:url value="/admin/noticeBoWriteView" />">
									<button id="writeBoard" type="button"
										class="btn btn-outline-info">공지 작성</button>
								</a>
							</div>
							<!-- 페이지네이션 -->
							<div>
								...
								<ul class="pagination"
									...
									<c:if test="${searchVO.firstPage !=1 }">
										<li class="page-item"><a class="page-link"
											href="mngrBoardView?curPage=${searchVO.firstPage-1 }"
									...
									<c:forEach begin="${searchVO.firstPage }"
										...
										<c:if test="${searchVO.curPage != i }">
											<li class="page-item"><a class="page-link"
												href="mnrgBoardView?curPage=${i}" data-page="${i}">${i}</a></li>
										</c:if>
										<c:if test="${searchVO.curPage == i }">
									...
									<c:if test="${searchVO.lastPage!=searchVO.totalPageCount }">
										<li class="page-item"><a class="page-link"
											href="mnrgBoardView?curPage=${searchVO.lastPage+1 }"
